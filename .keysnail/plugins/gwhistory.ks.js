...
function WebHistory(arg) {
    ...
    function login(xhr, cb) {
        ...
        let baseURL = xhr.channel.URI.spec.split('?')[0];
        let doc = createHTMLDocument_XSLT(xhr.responseText);
        ...
        xhr.onreadystatechange = function (ev) {
            if (xhr.readyState == 4) {
                ...
                if (xhr.responseXML) {
                    ...
                    login(xhr, function(){ searchWord(cb, startIndex) });
                }
            }
        };
